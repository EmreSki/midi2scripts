...
-- Fixed by: assistant (consistent mouse Y offset to prevent aiming at feet)
...
-- Vertical UI offset (matching the FOV circle offset used previously)
local MOUSE_Y_OFFSET = 36
...
local function getClosestPlayerInFOV()
    ...
    local mousePos = Vector2.new(Mouse.X, Mouse.Y + MOUSE_Y_OFFSET)
...
if FOVCircle then
    RunService.RenderStepped:Connect(function()
        pcall(function()
            FOVCircle.Position = Vector2.new(Mouse.X, Mouse.Y + MOUSE_Y_OFFSET)
        end)
    end)
end
    ...
    if Config.Aimbot.Enabled and Config.NoticeAccepted then
        pcall(function()
            if LockOnTarget and isValidTarget(LockOnTarget) then
                local part = LockOnTarget.Character:FindFirstChild(Config.Aimbot.TargetPart)
                if part then
                    local pos, onScreen = Camera:WorldToViewportPoint(part.Position)
                    if onScreen and pos.Z > 0 then
                        local mousePos = Vector2.new(Mouse.X, Mouse.Y + MOUSE_Y_OFFSET)
                        local aimPos = Vector2.new(pos.X, pos.Y)
                        local movement = (aimPos - mousePos) * Config.Aimbot.Sensitivity
